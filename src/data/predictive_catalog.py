"""Predictive failure modes, their variables and their methodology - one shared catalog.

Moved here out of ``dashboard/components/predictive_config.py`` so Campbell AI can explain a
predictive risk with the same variable mapping the Predictivo tab shows, without ``src``
importing from ``dashboard``. That import direction is the one that must not exist: the API
container mounts ``./src``, ``./config`` and ``./data`` but reaches ``dashboard`` only through
the Dockerfile's ``COPY``, so a backend import of the UI package would work on a fresh build
and then silently serve stale code - the same reason recorded in
``src/campbell_ai/oil_limits.py``.

``predictive_config`` re-exports everything defined here, so every existing dashboard import
keeps working and there is exactly one definition.

What this catalog does and does not say, because the difference decides how an answer may be
written from it:

- It maps a failure mode to the variables the model **considers**. That is a documented
  association - not a measured contribution, and not a cause.
- The mapping is **per client and per component**. CDA's oil degradation uses viscosity and
  soot; iron belongs to abrasive wear. Combustion and thermal-imbalance signals differ by
  client. Reading one client's mapping onto another is how "high Fe" gets attributed to a mode
  that never used iron.
- Capstone's entries are functional placeholders pending domain validation, as the original
  module recorded. They are not confirmed domain facts.
"""
